...
export class NewsTopicsService {
  ...
  async getOrCreateByNames(
    ...
    await this.repository.addTopic(normalizedNames, manager);

    return this.repository.findByNames(normalizedNames, manager);
  }

  async getActiveById(id: number, manager: EntityManager): Promise<NewsTopic> {
    const topic: NewsTopic | null = await this.repository.findActiveById(
      id,
      manager,
    );
