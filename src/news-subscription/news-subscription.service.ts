...
export class NewsSubscriptionService {
  ...
  async subscribe(telegramId: string, topicId: number): Promise<NewsTopicDto> {
    return this.dataSource.transaction(
      async (manager: EntityManager): Promise<NewsTopicDto> => {
        const topic: NewsTopic = await this.topicsService.getActiveById(
          topicId,
          manager,
        );

        const user: TelegramUser =
          await this.usersService.getOrCreateByTelegramId(telegramId, manager);

        if (!user.isActive) {
          throw new Error('Пользователь неактивен');
        }

        await this.repository.insertIfMissing(user.id, manager);

        const lockedSubscription: NewsSubscription | null =
          await this.repository.findForUpdate(user.id, manager);
        ...
        const subscription: NewsSubscription =
          await this.repository.findWithTopics(lockedSubscription.id, manager);

        const alreadySubscribed: boolean = subscription.newsTopics.some(
          (existingTopic: NewsTopic): boolean => existingTopic.id === topic.id,
        );
        ...
        await this.repository.save(subscription, manager);
  ...
  async unsubscribe(
    ...
    return this.dataSource.transaction(
      async (manager: EntityManager): Promise<NewsTopicDto | null> => {
        const user: TelegramUser | null =
          await this.usersService.findByTelegramId(telegramId, manager);
        ...
        const removedTopic: NewsTopic | undefined =
          subscription.newsTopics.find(
            (topic: NewsTopic): boolean => topic.id === topicId,
          );
        ...
        subscription.newsTopics = subscription.newsTopics.filter(
          (topic: NewsTopic): boolean => topic.id !== topicId,
        );
  ...
  async getSubscribedTopics(telegramId: string): Promise<NewsTopicDto[]> {
    const subscription: NewsSubscription | null =
      await this.repository.findActiveByTelegramId(telegramId);
    ...
    const topics: NewsTopic[] = subscription.newsTopics
      ...
      .sort((left: NewsTopic, right: NewsTopic): number =>
