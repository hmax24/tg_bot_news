...
describe('Subscription row lock', (): void => {
  it('locks only the subscription and parameterizes the user ID', async (): Promise<void> => {
    const source: MetadataDataSource = new MetadataDataSource({
      type: 'postgres',
      entities: [NewsSubscription, NewsTopic, TelegramUser],
    });
    await source.prepareMetadata();
    const repository: Repository<NewsSubscription> =
      source.getRepository(NewsSubscription);
    const builder: SelectQueryBuilder<NewsSubscription> =
    ...
    const subscriptions: NewsSubscriptionRepository =
      new NewsSubscriptionRepository(repository);
